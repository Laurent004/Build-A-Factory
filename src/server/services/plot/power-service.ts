...
import { PowerLineData, STRUCTURES } from "shared/constants/structures";
...
export default class PowerService implements OnInit, OnStart {
	...
	constructor(private readonly components: Components, private readonly factoryService: FactoryService) {}
	...
	private initEvents(): void {
		EventBus.PlotEvents.OnPlotReset.Connect(() => {
	...
	public attemptDisconnect(player: Player, startAttachment: Attachment, endAttachment: Attachment): void {
		...
		Events.OnPowerLineDestroying.broadcast(player, powerLine.Attachment0!, powerLine.Attachment1!);
		powerLine.Destroy();
	}
	...
	public getPowerLinesData(player: Player): PowerLineData[] {
		return (
			Workspace.WaitForChild("Plots")
				.GetChildren()
				.find((plot): plot is Model => plot.GetAttribute("UserId") === player.UserId)!
