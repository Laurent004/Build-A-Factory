...
export function InfoPanelPowerGraph({attachment}: InfoPanelPowerGraphProps) {
	...
	return (
		<Frame
			Position={UDim2.fromScale(0, .099)}
			Size={UDim2.fromScale(1, 0.901)}
			BackgroundTransparency={1}
		>
			<Frame
				ref={frameRef}
				Size={UDim2.fromScale(1, 0.717)}
				BackgroundTransparency={1}
			>
				{attachmentHistory!==undefined?[0, 1, 2, 3].map((value) => 
						attachmentHistory.map((powerNetworkInfo) => powerNetworkInfo[value]).map((point, index, points) => {
						if (index === points.size() - 1) return undefined;
						const startX=(index*(frameRef.current?.AbsoluteSize.X??0)) / (attachmentHistory).size()
						const endX = ((index+1)*(frameRef.current?.AbsoluteSize.X??0)) / (attachmentHistory).size();
						const startY =
			...
			<Frame
				Position={UDim2.fromScale(0,.772)}
						...
						<Text
							...
							Text={`${
								...
								value === 0
									...
									? "rgb(255,255,255)"
									: "rgb(79,79,79)"
							}" weight="regular">${attachmentHistory !== undefined && attachmentHistory.size() > 0
...
function InfoPanelPowerGraphSegment({
	value,
	position,
	rotation,
	size,
}: InfoPanelPowerGraphSegmentProps) {
	return (
		<Frame
