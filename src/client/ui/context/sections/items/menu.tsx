		...
		>
			...
			>
				...
				<Frame
					...
					Position={UDim2.fromScale(.639,.394)}
					Size={UDim2.fromScale(.573,.631)}
					BackgroundTransparency={1}
				>
					<uilistlayout
					...
					<Frame
						...
						BackgroundTransparency={1}
						LayoutOrder={2}
						Visible={ITEMS[selectedItemName].energy>0}
					>
						<uilistlayout
							Padding={new UDim(0,8)}
						...
						<NumberSpinner
							...
							TextSize={13}
							TextXAlignment={Enum.TextXAlignment.Left}
							value={ITEMS[selectedItemName].energy}
							suffix=" MJ"
						></NumberSpinner>
					</Frame>
					...
					<Frame
						...
						BackgroundTransparency={1}
						LayoutOrder={3}
						Visible={ITEMS[selectedItemName].value!==undefined}
					>
						<uilistlayout
							Padding={new UDim(0,8)}
							FillDirection={Enum.FillDirection.Horizontal}
						...
						>
							...
							<NumberSpinner
								...
								FontFace={fonts.josefinSans.bold}
								TextSize={13}
								value={[ITEMS[selectedItemName].value!.logisticsData,ITEMS[selectedItemName].value!.productionData,ITEMS[selectedItemName].value!.powerData].find((value)=>value>0)??0}
								commas={true}
							></NumberSpinner>
						</Frame>
					</Frame>
			...
			>
				...
				{Object.entries(ITEM_RECIPES)
							...
							>
								...
								>
									...
									{Object.entries(recipeDefinition.inputItems)
										...
										.map(([itemName, count], index) => (
											<Frame
												...
												LayoutOrder={index}
												Event={{MouseEnter:()=>{
													store.setTips([itemName,ITEMS[itemName].energy > 0? `Energy : ${ITEMS[itemName].energy} MJ`: undefined,`<font weight="regular" color="rgb(176,208,255)">${
																			math.floor(60 / recipeDefinition.time) * count
																		}</font> per minute`].filterUndefined())
									...
									<Frame
										Size={UDim2.fromScale(0.095, 0.75)}
										BackgroundColor3={Color3.fromRGB(42, 42, 42)}
										LayoutOrder={Object.entries(recipeDefinition.inputItems).size() + 1}
										Event={{MouseEnter:()=>{
									...
									<Image
										Size={UDim2.fromScale(0.064, 0.414)}
										LayoutOrder={Object.entries(recipeDefinition.inputItems).size() + 2}
										Image="rbxassetid://87835167641652"
										ImageColor3={colors.lightblue}
									>
										<Image
					...
					<Frame Size={UDim2.fromScale(0.428, 0.06)} BackgroundColor3={colors.white} LayoutOrder={Object.entries(ITEM_RECIPES)
					.filter(([, recipeDefinition]) => recipeDefinition.outputItems[selectedItemName] !== undefined).size()+2} Visible={Object.entries(ITEM_RECIPES)
					.filter(([, recipeDefinition]) => recipeDefinition.inputItems[selectedItemName] !== undefined).size()>0}>
						<uigradient
							Color={
								new ColorSequence([
									new ColorSequenceKeypoint(0, Color3.fromRGB(21, 21, 21)),
					...
					{Object.entries(ITEM_RECIPES)
					.filter(([, recipeDefinition]) => recipeDefinition.inputItems[selectedItemName] !== undefined).sort(([,recipeDefinitionA],[,recipeDefinitionB])=>recipeDefinitionA.index<recipeDefinitionB.index)
					.map(([recipeName, recipeDefinition],index) => (
						<Frame Size={UDim2.fromScale(1, 0.3)} BackgroundTransparency={1} LayoutOrder={Object.entries(ITEM_RECIPES)
					.filter(([, recipeDefinition]) => recipeDefinition.outputItems[selectedItemName] !== undefined).size()+index+3}>
							<Frame
								AnchorPoint={new Vector2(0.5, 0.5)}
								Position={UDim2.fromScale(0.5, 0.5)}
								Size={UDim2.fromScale(0.92, .79)}
