...
import { SplitterOutputFilter } from "./smart-splitter";
...
export class ProgrammableSplitterComponent extends TransporterComponent implements OnStart {
	...
	private updateOutputFilters(outputDirection: SplitterOutputDirection) {
		this.outputsFilters.set(
			outputDirection,
			HttpService.JSONDecode(this.instance.GetAttribute(outputDirection) as string) as SplitterOutputFilter[],
		);
	}
	...
	private getOutputTransportersOfFilter(outputFilter: SplitterOutputFilter): TransporterComponent[] {
		...
		for (const outputDirection of splitterOutputDirections.filter((outputDirection) =>
			this.outputsFilters.get(outputDirection)?.includes(outputFilter),
		)) {
			...
			[
				anyUndefinedOutputTransporters.size() > 0 &&
					splitterOutputDirections.every(
						(outputDirection) => !this.outputsFilters.get(outputDirection)!.includes(this.items[0].Name),
					),
