...
export const selectContextStructureComponents = <T extends BaseComponent>(component: AbstractConstructorRef<T>) =>
	createSelector([selectContextStructureModels], (structuresModels) => {
		if (structuresModels.size() === 0) return [];
		const components = Dependency<Components>();
		return [structuresModels[0], ...structuresModels[0].GetDescendants()]
